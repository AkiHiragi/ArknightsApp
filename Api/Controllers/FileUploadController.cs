    ...
    IImageProcessingService imageServiсe) : ControllerBase
{
        ...
        int previewX, int previewY, int previewWidth, int previewHeight)
    {
        ...
        Console.WriteLine(
            $"previewX: {previewX}, previewY: {previewY}, previewWidth: {previewWidth}, previewHeight: {previewHeight}");
        ...
        if (avatarSize <= 0 || previewWidth <= 0 || previewHeight <= 0)
            return BadRequest(
                $"Invalid crop parameters: avatarSize={avatarSize}, previewWidth={previewWidth}, previewHeight={previewHeight}");
